---
imagePath: /images/work/CaseStudyImages_5
---

[jobbox]
title: 490 Duboce Ave. Gate
client: SF Planning Department
role: Designer / Project Manager
delivery: Steel and Glass Gate

[subtitle]
Creative skills are handy for more[br] 
than just graphics and video.

[label]
THE SITUATION

[paragraph]
In 2018 – against all odds – my partner and I purchased a home in San Francisco. What we didn’t know was that we had also bought into a dispute with the City Planning Department over un-permitted construction. 

When we tried to get our own permits to perform desperately needed maintenance we found ourselves embroiled in a multi-year saga to appease the City planners. A chimera of physical, regulatory, aesthetic, practical and relationship challenges.

[pullquote]
Design is a framework
for <any> kind of problem

[label]
THE SOLUTION

[paragraph]
Much of what I produce is just colored light on a screen – but here was an opportunity to test my design skills in a new medium: steel and civic codes. We attacked the problem like any design challenge. We began by trying to understand the problem and our audience. Questions, research and conversation gave shape to the problem and its solution. 

We were tasked with meeting very specific city requirements  – San Francisco’s building codes are complex and highly variable. We had to collaborate with architects and our case manager to fully understand what was required of us and how our options were constrained.

Our space provided it’s own challenges – the existing arbor could not withstand the weight of a gate and would have to be rebiult. A concrete wall dividing us from our neighbor would need to factor into the design. 

[pullquote]
It wasn’t a requirement[br]
but <why not> make[br]
it beautiful also?[br]

[paragraph]
And we weren’t just trying to check off practical considerations, we had aesthetic goals too – we wanted a beautiful gate that would match our vision for the property and align with the aesthetic of the neighborhood. We aligned on a concept that would incorporate stained glass using the colors from the surrounding buildings.

From here we launched into a series of digital prototypes that moved through a series of creative software: Adobe Illustrator, Photoshop and even After Effects were key to creating mockups we could take to an architect. I created an expression drive project that allowed my non-technical partner to experiment with different color combinations and patterns.

With a clever architectural plan in place I moved into 3D modeling – ensuring that not only would the gate fit the actual space– but that it wouldn’t prevent us from parking a car in the future. I built and rebuilt the gate many times virtually before we hired a metal worker to start building it and a contractor to actually install it in the real world.

[pullquote]
From pixels into steel

[paragraph]
The end product is one of my favorite pieces of work – a testament to everything I have learned up to this point. The transformation of a digital design into a beautiful physical object is a truly satisfying outcome. 

[gallery]
GateGallery
4up, 4by3
